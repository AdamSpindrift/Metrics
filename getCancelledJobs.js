...
const getCancelledJobs = async () => {
    ...
    console.log("Getting Cancelled Jobs");
        ...
        for (let i = 0; i < allCompanies.length; i++) {
            ...
            Company.findOne({Name : allCompanies[i].Name}, async function(err, foundCompany){
                ...
                for(let i=0; i < clients.length; i++) {
                    ...
                    await axios.get("https://" + foundCompany.Name + ".senta.co/api/jobs?cid=" + clients[i]._id + "&status=cancelled" , { "headers" : {"accept": "application/json", "x-auth": apiKey }})
                            ...
                            for (let i = 0; i < foundJobs.length; i++) {
                                ...
                                if(readyJobs.includes(foundJobs[i]._id)) {
                                    ...
                                    Job.findOneAndUpdate({_id: foundJobs[i]._id}, {
                                        $set: {Status: "cancelled" , CompletedDate: completedDate}}, function (err) {
                                        if(err) {             
                                            console.log("Update Job Error: " + err + " Here.");
                                            throw (err);
                                        }; 
                                    });
                                };
