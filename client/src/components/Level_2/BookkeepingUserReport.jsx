...
function BookkeepingUserReport () {
    ...
    const [userAHR, setUserAHR] = useState(0);
    ...
    useEffect(() =>{
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                const urecover = userRecover.reduce(add,0);
                console.log(userRecover);
                console.log(urecover);
                const uAHR = userAHR.reduce(add, 0);
                setJobAnalysis(jobAnalysisBuild);
                setRecoverability(urecover/userRecover.length);
                setUserAHR((uAHR/userAHR.length));
                setLoading(false);
            },[200]);
    ...
    if( isLoggedIn === true) {
        ...
        return (
            <div className="container-2">
                <Header title="Bookkeeping User Report"/>
