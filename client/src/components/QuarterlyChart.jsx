    ...
    function regenerateData() {
        ...
        const getSeriesData = (employees, qFirstWeek, qLastWeek) => {
            employees.forEach((employee) => {
                const { VATReturn, PAYE, Bookkeeping, PersonalTaxReturn, AccountsProduction,
                        YTargetVATReturn, YTargetPAYE, YTargetBookkeeping, YTargetPersonalTaxReturn, YTargetAccountsProduction } = employee;
                ...
                vatTarget.push(YTargetVATReturn);
                payeTarget.push(YTargetPAYE);
                bookkeepingTarget.push(YTargetBookkeeping);
                ptrTarget.push(YTargetPersonalTaxReturn);
                accProdTarget.push(YTargetAccountsProduction);
            });
        ...
        if (todayValue >= startQuarterDays[0] && todayValue <= endQuarterDays[0]) {
            ...
            setTimeout(() => {
                ...
                chartData.push(vatTargetSeries);
                chartData.push(payeTargetSeries);
                chartData.push(bookkeepingTargetSeries);
                chartData.push(ptrTargetSeries);
                chartData.push(accProdTargetSeries);
            }, 50);
        ...
        if (todayValue >= startQuarterDays[2] && todayValue <= endQuarterDays[2]) {
            ...
            // Push Series
            setTimeout(() => {
                console.log("vat Targets Array - " + vatTargetSeriesArray);
                chartData.push(vatSeries);
                chartData.push(payeSeries);
                chartData.push(bookkeepingSeries);
                chartData.push(ptrSeries);
